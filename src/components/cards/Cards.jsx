...
const Cards = ({course,handleAdd}) => {
    ...
    return (
       ...
       <figure>
    <img className="w-[280px] h-[144px] rounded-xl mb-0" src={cover} alt={`${title} no ${id}`} />
        </figure>
     <div className="  m-4">
    <h2 className="card-title text-lg font-semibold  ">{title}</h2>
    <p className="font-normal text-sm mt-2 text-[grey]">{description}</p>
    <div className="grid grid-cols-2 gap-4 mt-4">
     <span className='flex gap-2'>
      ...
      </svg> <p className="text-base font-normal text-[grey]"> Price : {price}</p></span>  
     <span className='flex gap-2'><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
    ...
    </svg><p className="text-base font-normal text-[grey]">Credit : {credit} hr</p></span>   
    </div>
    <div className="card-actions justify-center mt-8 ">
      <button className="btn btn-primary px-[120px]" onClick={params} >Select</button>
    </div>
  </div>
</div>
        </div>
